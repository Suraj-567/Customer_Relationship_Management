 ...
 * Tickets Component
 * -----------------
 * This component manages the full lifecycle of ticket handling for employees.
 *
 * Key Features:
 * - Fetches all tickets from the backend and displays them in a searchable table.
 * - Allows viewing full ticket details in a modal (with description, attachments, assigned users, SLA status).
 * - Supports editing ticket fields such as title, description, assignedTo, and resolution message.
 * - Handles change detection to ensure only updated fields are submitted.
 * - Displays active and completed tickets, including SLA deadline and overdue indicators.
 * - Provides a responsive UI using TailwindCSS and Lucide icons.
 * - Includes modals for both viewing and editing with smooth state transitions.
 *
 * State Management:
 * - tickets:        Stores all fetched tickets.
 * - selectedTicket: Stores the ticket currently being viewed.
 * - editData:       Stores editable fields for the edit modal.
 * - searchQuery:    Handles live search filtering.
 * - loading flags:  Shows loaders for fetch and update operations.
 *
 * API Operations:
 * - GET  /api/tickets/fetch-tickets    -> Fetch all tickets
 * - PUT  /api/tickets/update/:id       -> Update edited fields of a ticket
 *
 * This component serves as the primary UI for managing employee ticket submissions.
 */
