 ...
 * ----------------------
 * This component displays a summarized company dashboard for the CRM Manager.
 * It fetches statistics such as total customers, employees, tickets raised,
 * and tickets resolved — then presents them in a responsive grid of cards.
 *
 * Major Features:
 * ---------------
 * ✔ Fetches company statistics using JWT-protected API  
 * ✔ Uses visually themed statistic cards (with Lucide icons)  
 * ✔ Each card is clickable and navigates to a detailed page  
 * ✔ Includes loading and error states for better UX  
 * ✔ Uses color configuration mapping to keep UI consistent  
 *
 * State Variables:
 * ----------------
 * - stats → Stores the fetched company statistics  
 * - error → Tracks API failure and triggers error UI  
 *
 * Data Fetching:
 * --------------
 * On component mount, an API request is sent to:
 *    GET /api/company/stats  
 * including the employee/manager token in the Authorization header.  
 *
 * Responses:
 * ----------
 * ✓ Success → Stores data in `stats` and renders dashboard cards  
 * ✗ Failure → Sets `error` to true and displays an error banner  
 *
 * UI Breakdown:
 * -------------
 * - A 4-card responsive grid showing:
 *      • Customers
 *      • Employees
 *      • Tickets Raised
 *      • Tickets Resolved
 * - Each card includes:
 *      • Label
 *      • Icon matching the color theme
 *      • Statistic value
 *      • “View Details” mini link
 *
 * Navigation:
 * -----------
 * Clicking a card redirects to its corresponding management page using `useNavigate()`.
 *
 * Error & Loading States:
 * -----------------------
 * - While loading → Shows full-screen "Loading statistics…"  
 * - On failure → Shows an error screen with an AlertTriangle icon  
 *
 * Purpose:
 * --------
 * This dashboard gives CRM Managers a real-time overview of company activity,
 * allowing them to quickly navigate to key sections and manage operations.
 */
