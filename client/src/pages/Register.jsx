 ...
 * ------------------------------
 * This component handles the complete company registration process,
 * including form management, API submission, validation, and UI/UX layout.
 *
 * Features:
 * - Uses a reusable <InputField /> component for consistent, icon-based inputs.
 * - Maintains all form state in a single formData object.
 * - Submits registration data to the backend using Axios.
 * - Shows real-time notifications using react-hot-toast (loading, success, errors).
 * - Redirects to the login page on successful registration.
 * - Displays a modern UI with TailwindCSS, Lucide icons, and a 3-column responsive grid.
 *
 * Important Behaviors:
 * - Prevents multiple submissions using a `loading` state.
 * - Disables all inputs while processing.
 * - Handles both basic company details AND SMTP configuration (email sending setup).
 * - Ensures required fields are validated in the browser.
 *
 * InputField Component:
 * ---------------------
 * - Defined outside the main component to prevent re-renders.
 * - Accepts icon, name, type, placeholder, value, onChange, and disabled props.
 * - Provides a uniform input design across the form.
 *
 * API Endpoint:
 * POST → /api/company/register
 * Body → { companyName, businessEmail, managerName, password, industry, address, phone, smtpUser, smtpPass, smtpHost, smtpPort }
 *
 * This page is used by companies to register their account before admin approval.
 */
