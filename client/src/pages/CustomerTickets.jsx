 ...
 * -------------------------
 * This component displays a full analytics dashboard of customers and their
 * ticket activity. It includes a summary table, animated charts, and a modal
 * showing detailed ticket history for each customer.
 *
 * Main Features:
 * --------------
 * ✔ Fetches aggregated customer ticket stats (total raised, resolved, etc.)
 * ✔ Displays an analytics dashboard with animated summary cards
 * ✔ Shows a customer-wise table including resolution rates and actions
 * ✔ Allows viewing full ticket history for a customer via a modal
 * ✔ Uses Framer Motion for animated transitions and UI effects
 * ✔ Provides detailed ticket view: priority, status, assigned users, audit trail
 *
 * State Variables:
 * ----------------
 * - summary: List of customer ticket statistics fetched from the backend
 * - selectedCustomer: Stores the customer currently opened in the modal
 * - customerTickets: All tickets belonging to the selected customer
 * - msg: General-purpose message state (currently unused but kept for future)
 *
 * API Calls:
 * ----------
 * GET /tickets/summary/customers
 *    → Fetches ticket count summary for every customer
 *
 * GET /tickets/customer/:customerId
 *    → Fetches complete ticket history of a single customer
 *
 * Utility Functions:
 * ------------------
 * - getPriorityColor(priority) → Returns Tailwind classes based on priority
 * - getStatusColor(status) → Returns Tailwind classes based on ticket status
 * - getResolutionRate(raised, resolved) → Calculates resolution percentage
 *
 * UI Structure:
 * -------------
 * 1️⃣ Header section — Page title + icon  
 * 2️⃣ Summary cards — Total customers, total tickets, resolved tickets  
 * 3️⃣ Customer table — Includes name, contact, totals, resolution rate  
 * 4️⃣ Customer modal — Appears when clicking “View History”
 *      • Customer details
 *      • Ticket statistics
 *      • Ticket list with animations
 *      • Audit trail for each ticket
 *
 * Design & Animation:
 * -------------------
 * - Fully styled with TailwindCSS
 * - Smooth scale, fade, and slide animations via Framer Motion
 * - Gradient-based dark theme dashboard layout
 * - Custom scrollbar styling for cleaner UI
 *
 * Purpose:
 * --------
 * This component is used by admins or employees to analyze customer-level
 * ticket performance and review historical ticket activity in detail.
 */
